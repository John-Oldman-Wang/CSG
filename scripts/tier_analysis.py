...
#
# ⚠️ **已剔除国联民生** —— 它 2024 年才吸并民生证券成为准头部，
# 而同花顺把现名回溯套用到 2011 年起的全部历史（707 条）。
# 那些研报实际由合并前的「国联证券」发出，当时是家中小券商。
# 它一度占发现期 T2 样本的 39%（239/619），足以左右结论。
#
# 这暴露一个更普遍的问题：**机构身份在时间上不稳定**，而两个数据源
# 的处理方式相反 —— 东财保留历史名（新时代证券 527 条），
# 同花顺回溯套用现名（诚通证券 513 条），同一批研报在两源下属于不同机构。
#
# 本分析的做法是**只用身份稳定的机构**：宁可样本小，
# 不要把「按今天的名字分档、拿历史数据检验」这种错误藏进结论里。
TIER2 = {"华泰证券", "招商证券", "广发证券", "东方证券",
         "光大证券", "中国银河", "兴业证券", "长江证券", "国信证券",
         "中泰证券"}

# 身份不稳定，一律排除出分档（不计入 T2 也不计入 T3）
UNSTABLE = {
    "国联民生": "2024 吸并民生证券；同花顺把现名回溯到 2011 年起的全部历史",
    "国联证券": "同上，合并前的名称",
    "申万宏源": "2015 申银万国与宏源合并",
    "中金财富": "原中投证券，2016 被中金收购、2019 更名",
    "中投证券": "同上，更名前的名称",
    "诚通证券": "原新时代证券，2021 更名",
    "新时代证券": "同上，更名前的名称",
    "国泰海通": "2025 国泰君安与海通合并",
